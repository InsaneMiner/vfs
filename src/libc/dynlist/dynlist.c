...
int dl_resize(dynlist *v, int capacity)
{
    ...
    if(v)
    {
        void **items = realloc(v->dl.items, sizeof(void *) * capacity);
        if (items)
        {
            v->dl.items = items;
...
int dl_push(dynlist *v, int size, void *item)
{
    void *ndata = malloc(size);

    memcpy(ndata, item, size);
...
int dl_set(dynlist *v, int index, int size,void *item)
{
    void *ndata = malloc(size);
...
void dl_init(dynlist **v)
{
    ...
    *v = malloc(sizeof(dynlist));
    ...
    (*v)->dl.total = 0;
    (*v)->dl.items = malloc(sizeof(void *) * (*v)->dl.capacity);
}